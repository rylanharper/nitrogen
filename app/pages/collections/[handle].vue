...
// Save the initial collection product values via `useState`
// This helps the filter-menu retain the correct filter and sort options across navigation and query changes
const initialProducts = useState<ProductFragment[] | null>('initialCollectionProducts', () => shallowRef(null))
...
watch(
  () => handle,
  () => {
    clearNuxtState('initialCollectionProducts')
    refresh()
  }
)
...
<template>
  <section v-if="collection" class="relative flex flex-col px-6">
    <div class="flex items-center justify-between gap-10 my-6">
